...
const ReportItem: React.FC<{ label: string; value: string | number | undefined | null; highlight?: boolean; large?: boolean }> = ({ label, value, highlight, large }) => (
    <div className={`flex justify-between items-center py-0.5 border-b border-[#333333] last:border-0 ${highlight ? 'bg-[#6D282C]/10 px-2 rounded -mx-2' : ''}`}>
        ...
        <span className={`font-bold ${large ? 'text-2xl text-[#ff8fa3]' : highlight ? 'text-base text-[#ff8fa3]' : 'text-base text-gray-100'}`}>{value ?? '--'}</span>
    </div>
);
...
const PreOpReportPage: React.FC = () => {
    const {
        patients,
        currentPatientId,
    ...
    const handlePrint = () => {
        printCurrentPage('Robotrix_PreOp_Report');
    };
    ...
    return (
        <div className="relative h-full flex flex-col overflow-hidden bg-gradient-to-br from-[#1E1E1E] to-[#121212]">
            ...
            <div className="flex justify-between items-center no-print px-4 pt-1 relative z-10 shrink-0">
                <h2 className="text-3xl font-extrabold text-[#E0E0E0] tracking-tight uppercase">PRE – OP Surgical Report</h2>
            </div>
                    ...
                    <ReportCard title="Surgical Decision Matrix" className="h-full border-t-4 border-t-[#6D282C]">
                        <div className="space-y-2">
                            ...
                            <div className="space-y-2 mt-1">
                                ...
                                <div className="flex items-center justify-between p-2 bg-[#6D282C]/20 border border-[#6D282C]/50 rounded-lg">
                                    <span className="text-gray-400 text-sm font-medium">Recommended Tibial Cut</span>
...
export default PreOpReportPage;
