...
const LongLegCoronalBalancingPage: React.FC = () => {
    const { 
        ...
        lateralLaxity,
        longLegCoronalBalancingMainImage, setLongLegCoronalBalancingMainImage,
        longLegCoronalBalancingBlockImage, setLongLegCoronalBalancingBlockImage
    } = useAppContext();
    ...
    const handleMainUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onload = (ev) => setLongLegCoronalBalancingMainImage(ev.target?.result as string);
    ...
    const handleBlockUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            const reader = new FileReader();
            reader.onload = (ev) => setLongLegCoronalBalancingBlockImage(ev.target?.result as string);
            reader.readAsDataURL(e.target.files[0]);
        }
    };
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex justify-between items-center mb-6 no-print">
                <h2 className="text-4xl font-bold text-gray-100">Coronal Balancing screen (Long Leg)</h2>
                 <button onClick={() => setPage('results-analysis')} className="gemini-dark-button font-bold py-2 px-4 rounded-md transition text-md flex items-center space-x-2">
            ...
            <div className="flex flex-col lg:flex-row flex-grow gap-6">
                ...
                <div className="w-full lg:w-1/4 gemini-dark-card p-6 rounded-lg flex flex-col items-start text-left">
                    ...
                    >
                        {longLegCoronalBalancingBlockImage ? (
                            <img src={longLegCoronalBalancingBlockImage} alt="AI Block" className="w-full h-full object-cover" />
                        ) : (
                ...
                <div className="w-full lg:w-2/4 flex flex-col gap-4">
                    <div className="w-full relative flex-grow min-h-[450px] flex items-center justify-center bg-transparent overflow-hidden group">
                        ...
                        <div 
                            ...
                            onClick={() => !longLegCoronalBalancingMainImage && fileInputRef.current?.click()}
                        >
                            {longLegCoronalBalancingMainImage ? (
                                <img src={longLegCoronalBalancingMainImage} alt="Knee View" className="w-full h-full object-contain" />
                            ) : (
